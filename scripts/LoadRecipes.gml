...
while(!file_text_eof(file))
{
    ...
    if(string_pos("recipe", line) != 0)
    {
        ...
        recipe.name = string_copy(line, 8, string_length(line) - 9)
    ...
    {
        // Add the first tag
        recipes[iterator].tags[0] = string_copy(line, 6, string_length(line) - 7)
    }
    else if(string_pos("tag", line) != 0)
    {
        // Add a new tag to the end of the required tags
        recipes[iterator].tags[array_length_1d(recipes[iterator].tags)] = string_copy(line, 5, string_length(line) - 6)
    }
